%This example from Strogatz-Nonlinear-Dynamics and chaos
...
clear
syms x y a b s T
eq1=T*(a-x-(4*x*y)/(1+x^(2))); %First equation ODE RHS
eq2=T*(b*x*(1-y/(1+x^(2))));  %Second equation ODE RHS
syms x0 y0 xc yc omega
k=1; % number of cos & sin components in harmonic expansion of x
xc=sym('xc',[1,k]);
yc=sym('yc',[1,k]);% unknown harmonic coefficients (cos)
...
xe=x0+cos((1:k)*s)*xc+sin((1:k)*s)*xs;% expansion of x(t)
xe1=y0+cos((1:k)*s)*yc
dxe=diff(xe,s); % derivative xe
dye=diff(xe1,s);% derivative xe1
syms x1 y1 T1 b1 xh yh Th bh  %xhat=x1 in report
SU=subs([eq1  eq2],[x  y  T   b   a],[2+x1  5+y1  T1+(1/sqrt(sym(7)))    (7/2)+b1   10]) 
TA=taylor(SU,[x1  y1  T1   b1 ],[0  0  0  0],'order',3)
deq=subs(TA,[x1  y1],[xe,xe1])
S=[1;reshape([cos(s*(1:k)),sin(s*(1:k))],[],1)]
S1=simplify(S*[deq(1)-dxe , deq(2)-dye])
aeq=(int(S1,s,0,2*pi))/(2*pi)  %Taking integral from 0 to 2 pi
S2=simplify(aeq)
vars=[x0;xc(1);y0;yc(1);xs(1);T1;b1] % list of unknowns
sgvals=[0;0;0;0;0;0;0] 
residual=subs(aeq,vars,sgvals);
J0=jacobian(aeq(:),vars) 
J1=subs(J0,vars,sgvals); % Jaocbian is singular
V=null(J1)
